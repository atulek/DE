...
class DE:
    ...
    def __init__(self, test_func, repeat=100, maxit=100, npop=20, nvar=50, F=0.2, cp=0.2):
        ...
        for rpt in range(self.repeat):
            ...
            for i in range(self.npop):
                ...
                pop[i]["position"] = np.random.uniform(
                    self.varmin, self.varmax, self.nvar
                )
            ...
            bests = np.empty(self.repeat)
            print(bestcost[-1])
            bests[rpt] = bestcost[-1]
            print("-----------------------------------")
            print(bests)
